...
> {
	...
	match yard.try_create_new_ident(walker, new_token) {
...
impl ShuntingYard {
	...
	fn try_create_type_specifier<'a, P: TokenStreamProvider<'a>>(
		...
		walker: &mut Walker<'a, P>,
		ctx: &mut Ctx,
	) -> Result<(super::ast::Type, Vec<Syntax>), Option<Expr>> {
		use super::{
			ast::{Type, TypeTy},
			StructHandle,
		};
